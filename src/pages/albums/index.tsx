...
import { getAlbums, deleteAlbumById } from 'apiSdk/albums';
import { AlbumInterface } from 'interfaces/album';

type ColumnType = ColumnDef<AlbumInterface, unknown>;
...
export function AlbumListPage(props: AlbumListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getAlbums({
        relations: ['organization', 'user', 'album_song.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AlbumInterface>>(
    () => `/albums?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AlbumInterface) => {
    if (hasAccess('album', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/albums/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('album_song', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'album_song',
          header: 'Album Song',
          accessorKey: 'album_song',
          cell: ({ row: { original: record } }: any) => record?._count?.album_song,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/albums/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('album', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/albums/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('album', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="album" />
        </Flex>

        {hasAccess('album', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/albums/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
