...
import { getAlbumSongs } from 'apiSdk/album-songs';
import { AlbumSongInterface } from 'interfaces/album-song';

import { AlbumListPage as ShowCaseList } from 'pages/albums';

function DashboardPage() {
  const { session } = useSession();

  const songStat = useSWR<PaginatedInterface<SongInterface>>(
    () => `/songs`,
    () => getSongs({ limit: 1, offset: 0 }),
  );
  ...
  const albumSongStat = useSWR<PaginatedInterface<AlbumSongInterface>>(
    () => `/album-songs`,
    () => getAlbumSongs({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Songs',
      result: songStat?.data?.totalCount,
      isLoading: songStat.isLoading,
    ...
    {
      title: 'Album Songs',
      result: albumSongStat?.data?.totalCount,
      isLoading: albumSongStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
